...
const Projects = () => {
  ...
  return (
    <div className={`d w-screen h-screen flex justify-center items-center relative flex-col ${onView ? "overflow-hidden": ""}`}>
      ...
      <div className=" w-4/5 h-4/5 grid grid-cols-1 grid-rows-2 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-2">
        {projectList.map((item) => (
          <ProjectItem setOnView={setOnView} {...item} />
        ))}
      </div>

      {onView ? <LayoutModal setOnView={setOnView} children={"dasdasd"}/> : null}
    </div>
  );
};
