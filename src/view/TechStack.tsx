...
const TechStack = () => {
  ...
  return (
    <div className=" w-full h-full bg-orange-700 flex flex-col justify-center items-center">
      <div className=" p-4 w-full">
        <p className=" ml-32 font-custom1 text-2xl font-medium">Technologies</p>
      </div>
      <div className=" w-4/5 h-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-8 grid-rows-1 gap-2 mt-2 border-slate-950">
        {handleFilterList(techStackList, "front").map((item) => (
          <TechStackItem {...item} />
        ))}
      </div>

      <div className=" w-4/5 h-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-8 grid-rows-1 gap-2 mt-2">
